...
function ScrollReveal({ children, className = "", delay = 0, threshold = 0.1 }: { children: ReactNode; className?: string; delay?: number; threshold?: number }) {
  const ref = useRef<HTMLDivElement>(null);
...
function TiltCard({ children, className = "" }: { children: ReactNode; className?: string }) {
  const ref = useRef<HTMLDivElement>(null);
  const [style, setStyle] = useState<React.CSSProperties>({ transform: "perspective(1000px) rotateX(0deg) rotateY(0deg) scale3d(1, 1, 1)" });
  ...
  <div className="flex items-center gap-3 group cursor-pointer">
    <div className="shrink-0 w-8 h-8 md:w-10 md:h-10 flex items-center justify-center transition-transform duration-500 group-hover:rotate-[180deg]">
      <svg width="100%" height="100%" viewBox="0 0 32 32" fill="none">
        ...
        <path d="M12 10 Q7 16 12 22" fill="none" stroke="#2563eb" strokeWidth="2.5" strokeLinecap="round" opacity="0.9"/>
        <path d="M9 7 Q2 16 9 25" fill="none" stroke="#2563eb" strokeWidth="2" strokeLinecap="round" opacity="0.5"/>
        <path d="M20 10 Q25 16 20 22" fill="none" stroke="#60a5fa" strokeWidth="2.5" strokeLinecap="round" opacity="0.9"/>
        <path d="M23 7 Q30 16 23 25" fill="none" stroke="#60a5fa" strokeWidth="2" strokeLinecap="round" opacity="0.5"/>
      </svg>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-[1000] bg-black/80 backdrop-blur-xl flex items-center justify-center p-4 md:p-6 animate-fadeIn" onClick={onClose}>
      <div className="bg-[#111113] border border-zinc-800 rounded-3xl p-6 md:p-8 text-center max-w-sm w-full shadow-2xl relative overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className="text-2xl font-bold text-zinc-50 mb-3 tracking-tight">Deployment Pending</h2>
        <p className="text-zinc-400 text-sm leading-relaxed mb-8">
          The standalone desktop and mobile applications are currently undergoing testing. Please use the web platform in the meantime.
        </p>
        <button 
...
const FEATURES = [
  ...
  { icon: <ImageIcon className="text-blue-400" size={24} />, title: "Media Management", desc: "Secure cloud infrastructure for sharing documents, images, and videos with automatic format optimization and compression." },
  { icon: <Lock className="text-blue-400" size={24} />, title: "Advanced Security", desc: "Rigorous security standards including strict JWT sessions, automated threat blocking, and optional Two-Factor Authentication." },
  { icon: <Users className="text-blue-400" size={24} />, title: "Scalable Workspaces", desc: "Support for direct messaging and large-scale group environments, complete with administrative controls and pinning features." },
  { icon: <Palette className="text-blue-400" size={24} />, title: "Adaptive Interface", desc: "A precision-engineered user interface offering seamless transitions between intelligent light and dark modes." },
];
...
export default function LandingPage() {
  ...
  const heroScale = Math.min(1, 0.90 + scrollProgress * 0.10);
    ...
    <>
      ...
      <div className="font-sans bg-[#050505] text-zinc-100 min-h-screen overflow-hidden relative selection:bg-blue-500/30 selection:text-blue-200">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 hidden md:block">
          <Parallax speed={0.15} className="absolute top-[15%] left-[10%] opacity-[0.03]">
            <MessageSquare size={120} />
          </Parallax>
          <Parallax speed={0.25} className="absolute top-[40%] right-[12%] opacity-[0.03]">
            <Lock size={140} />
          </Parallax>
          <Parallax speed={0.1} className="absolute top-[75%] left-[15%] opacity-[0.03]">
            <Globe size={100} />
          </Parallax>
        </div>
        ...
        <section className="relative pt-32 pb-16 md:pt-48 md:pb-24 px-4 md:px-6 flex flex-col items-center text-center z-10 min-h-[90vh] md:min-h-0">
          ...
          <div className="w-full max-w-5xl mx-auto mt-16 md:mt-24 px-2 md:px-0 perspective-1000">
            <div 
              style={{ transform: `rotateX(${heroRotateX}deg) scale(${heroScale})`, transition: "transform 0.1s ease-out" }}
              ...
              <div className="flex-1 flex p-4 md:p-6 gap-6 relative">
                ...
                <div className="flex-1 flex flex-col justify-end gap-4 relative pb-2">
                  <div className="self-start max-w-[85%] md:max-w-[70%] bg-white/5 border border-white/10 rounded-2xl rounded-tl-sm p-3 md:p-4 text-sm text-zinc-300">
                    The deployment was successful. Let's sync on the new architecture.
                  </div>
                  <div className="self-end max-w-[85%] md:max-w-[70%] bg-blue-600 text-white rounded-2xl rounded-tr-sm p-3 md:p-4 text-sm">
                    Reviewing the logs now. Everything looks stable across all nodes.
                  </div>
                  <div className="self-start max-w-[85%] md:max-w-[70%] bg-white/5 border border-white/10 rounded-2xl rounded-tl-sm p-3 md:p-4 text-sm text-zinc-300">
                    Excellent. Initiating the secure video bridge for the review.
                  </div>
        ...
        <div className="relative z-20 py-4 bg-zinc-900 border-y border-zinc-800 overflow-hidden flex items-center">
          <div className="flex w-max animate-marquee items-center">
            {[...Array(4)].map((_, idx) => (
              <div key={idx} className="flex gap-8 md:gap-16 items-center px-8 text-xs md:text-sm font-bold text-zinc-500 uppercase tracking-widest whitespace-nowrap">
                <span className="flex items-center gap-2"><Zap size={16}/> Zero Latency</span>
                <span>•</span>
                <span className="flex items-center gap-2"><Lock size={16}/> Encrypted Data</span>
                <span>•</span>
                <span className="flex items-center gap-2"><Activity size={16}/> WebRTC Ready</span>
                <span>•</span>
                <span className="flex items-center gap-2"><Shield size={16}/> 2FA Security</span>
                <span>•</span>
              </div>
            ))}
          </div>
        ...
        <section className="relative z-10 py-20 md:py-28 bg-[#050505]">
          <div className="max-w-6xl mx-auto px-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 text-center">
              {STATS.map((s, idx) => (
                <ScrollReveal key={s.label} delay={idx * 100} className="relative">
                  <div className="text-4xl md:text-5xl font-black text-white tracking-tight mb-2">{s.value}</div>
                  <div className="text-xs md:text-sm text-zinc-500 font-bold uppercase tracking-widest">{s.label}</div>
                </ScrollReveal>
              ))}
            </div>
          </div>
        ...
        <section className="px-4 md:px-8 py-20 md:py-32 max-w-7xl mx-auto relative z-10 border-t border-white/5">
          <ScrollReveal className="text-center mb-16 md:mb-24">
            ...
            <p className="text-zinc-400 text-base md:text-lg max-w-2xl mx-auto">
              A robust feature set built to facilitate reliable and secure communication.
            </p>
          </ScrollReveal>
        ...
        <section className="px-4 md:px-8 py-20 md:py-28 relative z-10 bg-zinc-950/50 border-y border-white/5">
          <div className="max-w-4xl mx-auto">
            <ScrollReveal className="mb-16">
              <h2 className="text-3xl md:text-4xl font-black text-white mb-4">Implementation Workflow</h2>
              <p className="text-zinc-400 text-base">Initialize your workspace in three streamlined steps.</p>
            </ScrollReveal>
            ...
            <div className="relative border-l border-zinc-800 ml-4 md:ml-6 space-y-12 pb-4">
              {[
                ...
                <ScrollReveal key={idx} delay={150} className="relative pl-8 md:pl-10">
                  ...
                  <p className="text-zinc-400 leading-relaxed text-sm md:text-base max-w-xl">{step.desc}</p>
                </ScrollReveal>
              ))}
            </div>
          </div>
          ...
          <div className="relative max-w-3xl mx-auto z-10 glass-panel border border-white/10 rounded-[2rem] p-8 md:p-16 shadow-2xl">
            <ScrollReveal>
              ...
              <p className="text-zinc-400 max-w-xl mx-auto mb-10 text-base md:text-lg">
                Access the communication platform engineered for reliability and privacy. No complex configuration required.
              </p>
            </ScrollReveal>
        ...
        <footer className="relative z-10 border-t border-zinc-900 bg-[#050505]">
          <div className="px-6 md:px-10 py-12 max-w-7xl mx-auto">
            ...
            <div className="pt-8 border-t border-zinc-900 flex flex-col md:flex-row items-center justify-between gap-4 text-center md:text-left">
              <span className="text-xs text-zinc-600 font-medium">© 2026 VokiToki. All systems operational.</span>
              <span className="text-xs text-zinc-600 font-medium">MIT Licensed</span>
            </div>
          </div>
        </footer>
      </div>
