      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm shadow-2xl">
          <motion.div
            ...
            className="w-full max-w-sm bg-chat-bg-primary border border-chat-border rounded-2xl overflow-hidden"
          >
            <div className="p-6">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={onClose}
                  disabled={isLoading}
                  className="flex-1 py-2.5 px-4 rounded-xl text-sm font-semibold text-chat-text-secondary hover:bg-chat-hover transition-colors"
                >
                  {cancelText}
                </button>
                <button
                  ...
                  className={`flex-1 py-2.5 px-4 rounded-xl text-sm font-semibold text-white transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 ${
                    type === "danger"
                      ...
                      : "bg-chat-accent shadow-lg shadow-chat-accent/20"
                  }`}
                >
                  {isLoading ? (
