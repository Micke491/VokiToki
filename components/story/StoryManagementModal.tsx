    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-chat-bg-primary/90 backdrop-blur-2xl rounded-[var(--radius-lg)] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] border border-white/10 w-full max-w-4xl h-[85vh] overflow-hidden flex flex-col"
        >
          {/* Header */}
          <div className="flex items-center justify-between p-8 border-b border-white/5">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={onAddStory}
                disabled={uploading}
                className="px-6 py-3 bg-white text-black hover:bg-gray-200 active:scale-95 rounded-2xl text-sm font-bold transition-all flex items-center gap-2 shadow-xl disabled:opacity-50"
              >
                {uploading ? (
              ...
              <button
                onClick={onClose}
                className="p-3 bg-white/5 hover:bg-white/10 active:scale-90 border border-white/10 rounded-2xl text-white transition-all shadow-lg"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
                      ...
                      <div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-black/90 via-black/40 to-transparent">
                        <div className="flex items-center justify-between">
                          <div className="flex flex-col gap-1.5">
                             ...
                             <div className="flex items-center gap-1.5 text-xs font-medium text-gray-300 whitespace-nowrap">
                    ...
                    <div className="space-y-1">
                      {(() => {
                        ...
                        if (!story?.viewedBy?.length) {
                          return (
                            <div className="h-full flex flex-col items-center justify-center text-center text-gray-500 opacity-40 py-10">
                              <p className="text-xs font-semibold uppercase tracking-tight">No views yet</p>
                            </div>
                          );
                        }
                        ...
                        return uniqueViewers.map((view: any, idx) => (
                           <div key={idx} className="flex items-center justify-between p-3 rounded-2xl hover:bg-white/5 transition-colors group">
                              <div className="flex items-center gap-3">
                                 <div className="w-10 h-10 rounded-full bg-gradient-to-br from-chat-accent/20 to-chat-accent-secondary/20 flex items-center justify-center border border-white/10 overflow-hidden">
                                    {view.user?.avatar ? (
                                      <img src={view.user.avatar} className="w-full h-full object-cover" alt="" />
                                    ) : (
                                      <span className="text-chat-accent font-black text-xs">
                                        {(view.user?.username || 'U').charAt(0).toUpperCase()}
                                      </span>
                                    )}
                                 </div>
                                 <div className="flex flex-col text-left">
                                    <span className="text-sm font-bold text-white group-hover:text-chat-accent transition-colors">
